...
int main(/*int argc, char **argv*/)
{
  ...
  */
  ...
  // If IOException is thrown, close the serial port and exit.
    ...
    //Check if Serial Port was opened properly
    if(fd == -1){
...
//    
//    //Write to NGIMU to turn off all the unnecessary messages
//    char *turnOffMessages[5] = {"/auxserial, { Blob: 0x24505542582C34302C524D432C302C302C302C302A34370D0A }", "/auxserial, { Blob: 0x24505542582C34302C5654472C302C302C302C302A35450D0A }", "/auxserial, { Blob: 0x24505542582C34302C4747412C302C302C302C302A35410D0A }", "/auxserial, { Blob: 0x24505542582C34302C4753412C302C302C302C302A34450D0A }", "/auxserial, { Blob: 0x24505542582C34302C4753562C302C302C302C302A35390D0A }"};
//    int c;
//    int n;
//    for(c = 0; c < 5; c++){
//        printf("In for loop\n");
//        n = write(fd, turnOffMessages[c], 76);
//        if(n < 0){
//            printf("Failed to write command %d to IMU\n", c);
//        }
//    }
    
    fcntl(fd, F_SETFL, 0);
    
    //Configure the Baud Rate for the IMU
    ...
    //Read byte by byte and process each one 
    while(1){
        read(fd, byte, 1);
        //printf("%c\n", byte[0]);
...
*/
void ngimuReceiveErrorCallback(const char* const errorMessage) {
    std::cout << "Error Message Callback: " << errorMessage;
